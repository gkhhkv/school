...
> **核心定位**：Harness 是部署在工作区根目录的"监管层"。当你在同一工作区内创建任意 Spring Boot 子项目时，Harness 的 `.claude/CLAUDE.md`、hooks 和规范文档**自动对该子项目生效**，Claude Code 在整个研发生命周期中均受 Harness 约束，无需额外配置。

---

## 模板概述

Spring Boot Harness 工程管理模板通过 Claude Code 的原生机制（CLAUDE.md、hooks、规范文档）将工程标准内嵌到 AI 辅助开发流程中，实现**规范即代码、门禁即流程**的研发基线。
...
**步骤 3：在 Claude Code 中打开工作区根目录**

Claude Code 自动加载 `.claude/CLAUDE.md`，约束立即生效，无需任何额外配置。

**步骤 4：验证 CLAUDE.md 已存在**
...
向 Claude Code 发起任何开发请求，Claude 会自动执行 GATE-01 检查（确认需求文档是否存在），引导你走完整的 Dev_Lifecycle 六阶段流程。
...
Claude Code 在本工作区内遵循 Dev_Lifecycle 六阶段流程：
